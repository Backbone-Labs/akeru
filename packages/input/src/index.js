...
const freezeSnapshot = (value) =>
  Object.freeze({
    ...value,
    buttons: freezeRecord(value.buttons),
    axes: freezeRecord(value.axes),
  });
...
function listenerSet(reportError) {
  ...
  return {
    add(listener) {
      if (typeof listener !== 'function' || listeners.size >= 32)
        throw new TypeError('Invalid input listener');
...
export function createBrowserInputProvider(options = {}) {
  ...
  if (!window?.addEventListener || !document?.addEventListener)
    throw new TypeError('Browser event APIs are required');
  ...
  if (options.selectedController !== undefined)
    preferences.selectedController = options.selectedController;
  if (options.mapping?.gamepad)
    preferences.mappings.gamepad = validateMapping(
      'gamepad',
      options.mapping.gamepad,
    );
  if (options.mapping?.touch)
    preferences.mappings.touch = validateMapping(
      'touch',
      options.mapping.touch,
    );
  ...
  if (typeof raf !== 'function' || typeof cancelRaf !== 'function')
    throw new TypeError('Animation frame APIs are required');

  const reportError =
    typeof options.onError === 'function'
      ? options.onError
      : (error) => globalThis.console?.error?.(error);
  const snapshots = listenerSet(reportError),
    navigation = listenerSet(reportError),
    removers = [];
  let started = false,
    disposed = false,
  ...
  const time = () => {
    ...
    lastTime = Math.max(lastTime, Number.isFinite(value) ? value : 0);
    return lastTime;
  };
  const persist = () => {
    preferences = savePreferences(storage, options.titleId, preferences);
  ...
  const touchChanged = (phase, control, pointerId) => {
    ...
    else if (touchPointers.get(pointerId) === control)
  ...
  const listGamepads = () => {
    ...
    try {
      gamepadStatus = 'available';
      return Array.from(navigator.getGamepads() ?? [])
        .filter(Boolean)
        .filter((pad) => pad.connected !== false && pad.mapping === 'standard');
    } catch {
      gamepadStatus = 'unavailable';
      return [];
    }
  };
  const gamepadNeutral = (raw) =>
    controlsNeutral(raw, {
      axisThreshold: preferences.deadzone,
      buttonThreshold: 0.05,
    });
  const chooseGamepad = (pads) => {
    if (preferences.selectedController !== null)
      return (
        pads.find((pad) => pad.index === preferences.selectedController) ?? null
      );
    const current = pads.find((pad) => pad.index === activeGamepad);
    const engaged = pads.find((pad) => {
      const raw = readStandardGamepad(pad);
      return raw && !gamepadNeutral(raw);
    });
    if (engaged && (!current || gamepadNeutral(readStandardGamepad(current))))
      return engaged;
    return current ?? engaged ?? pads[0] ?? null;
  };
  const updateControllerList = (pads) => {
    const next = pads
      .map((pad) => Object.freeze({ index: pad.index, mapping: 'standard' }))
      .sort((a, b) => a.index - b.index);
    const signature = JSON.stringify(next);
    if (signature !== controllerSignature) {
      controllerSignature = signature;
      connectedControllers = next;
      ui?.update(preferences, connectedControllers);
    }
  };
  const gamepadNavigation = (controls, at) => {
    const pressed = {
      ...
      down:
        (controls.buttons.down ?? 0) > 0.5 || (controls.axes.moveY ?? 0) > 0.6,
      left:
        (controls.buttons.left ?? 0) > 0.5 || (controls.axes.moveX ?? 0) < -0.6,
      right:
        (controls.buttons.right ?? 0) > 0.5 || (controls.axes.moveX ?? 0) > 0.6,
    ...
    for (const [action, down] of Object.entries(pressed)) {
      ...
      if (!held) {
        ...
        ['up', 'down', 'left', 'right'].includes(action) &&
        at - held.started >= navRepeat.initialMs &&
        at - held.last >= navRepeat.intervalMs
      ) {
        held.last = at;
        emitNavigation({ type: 'move', direction: action });
      }
    }
  };
  const poll = () => {
      ...
      if (focused && raw) {
        if (gamepadNeedsNeutral) {
          if (gamepadNeutral(raw)) {
            ...
            if (ui && activeProvider === 'gamepad')
              emitSnapshot(
                'gamepad',
                true,
                normalizeRawControls(
                  raw,
                  preferences.mappings.gamepad,
                  preferences.deadzone,
                ),
                true,
              );
  ...
  const keydown = (event) => {
    ...
    emitNavigation(navigationEvent);
  };
  ...
  const api = {
    mount({ touchRoot, controlsRoot } = {}) {
      if (disposed || ui)
        throw new Error(
          disposed
            ? 'Input provider disposed'
            : 'Input provider already mounted',
        );
      resetInput();
      ui = createInputUi({
        document,
        ...
        onDeadzone: (value) => api.setDeadzone(value),
        onController: (value) => api.selectController(value),
        onGamepadMapping: (value) => api.setMapping('gamepad', value),
        onReset: () => api.resetPreferences(),
      });
      ui.update(preferences, connectedControllers);
      let mounted = true;
      return () => {
    ...
    subscribe(listener) {
      if (disposed) throw new Error('Input provider disposed');
      return snapshots.add(listener);
    },
    subscribeNavigation(listener) {
      if (disposed) throw new Error('Input provider disposed');
      return navigation.add(listener);
    },
    setDeadzone(value) {
      if (!Number.isFinite(value) || value < 0 || value > 0.5)
        throw new TypeError('Invalid deadzone');
      resetInput();
      preferences.deadzone = value;
      persist();
    },
    setMapping(provider, value) {
      const mapping = validateMapping(provider, value);
      resetInput();
      preferences.mappings[provider] = mapping;
      persist();
    },
    selectController(index) {
      if (
        index !== null &&
        (!Number.isSafeInteger(index) || index < 0 || index > 15)
      )
        throw new TypeError('Invalid controller index');
      resetInput();
      activeGamepad = null;
      preferences.selectedController = index;
      persist();
    },
    resetPreferences() {
      resetInput();
    ...
    getState() {
      return Object.freeze({
        ...
        activeProvider,
        activeController: activeGamepad,
        controllers: Object.freeze(connectedControllers.slice()),
      });
    },
    showControls() {
      ui?.show();
