...
export function preferenceKey(titleId) {
  if (
    ...
    titleId.length > 64 ||
    !titlePattern.test(titleId)
  )
    throw new TypeError('Invalid title id');
...
export function validatePreferences(value) {
  if (
    ...
    Object.keys(value).some(
      (key) =>
        !['version', 'deadzone', 'selectedController', 'mappings'].includes(
          key,
        ),
    ) ||
    value.version !== 1 ||
    ...
    value.deadzone < 0 ||
    value.deadzone > 0.5 ||
    (value.selectedController !== null &&
      (!Number.isSafeInteger(value.selectedController) ||
        value.selectedController < 0 ||
        value.selectedController > 15)) ||
    !value.mappings ||
    Object.keys(value.mappings).some(
      (key) => !['gamepad', 'touch'].includes(key),
    )
  )
    throw new TypeError('Invalid input preferences');
  return {
    version: 1,
...
export function loadPreferences(storage, titleId) {
  ...
  try {
    ...
    return encoded === null
      ? fallback
      : validatePreferences(JSON.parse(encoded));
  } catch {
    return fallback;
  }
}
...
export function savePreferences(storage, titleId, preferences) {
  ...
  try {
    storage.setItem(preferenceKey(titleId), JSON.stringify(checked));
  } catch {
    /* Input remains usable when storage is unavailable. */
  }
  return checked;
}
