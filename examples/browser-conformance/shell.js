...
addEventListener('message', event => {
  const expected = config.titles.find(t => t.origin === event.origin && document.getElementById(t.name).contentWindow === event.source);
  if (!expected || event.data?.nonce !== expected.nonce || event.data?.name !== expected.name || event.data?.type !== 'result') { invalidRejected++; return; }
  received.set(expected.name, event.data);
});
// Correct-looking payload from the shell is not accepted as title authority.
postMessage({ type: 'result', name: 'a', nonce: config.titles[0].nonce }, location.origin);
...
setTimeout(() => {
 ...
 clearInterval(poll);
 document.getElementById('result').textContent = JSON.stringify({ passed: false, error: 'Fixture timed out', received: received.size });
}, 10000);
