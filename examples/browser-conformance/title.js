...
const saved = await host.services.saves.write(
  'progress',
  { schemaVersion: 1, bytes: new Uint8Array([42]) },
  null,
);
const storageWorked =
  (await host.services.saves.read('progress')).bytes[0] === 42;
...
const presentationWorked =
  presentation.mode === 'embedded' && !presentation.granted;
...
const presentationStateWorked =
  initialPresentation.orientation === 'landscape' &&
  host.services.presentation.getState().safeArea.bottom === 20;
const audioBlocked = !(await host.services.audio.requestPlayback()).granted;
host.updateAudio({ state: 'ready', reason: null });
const audioWorked =
  audioBlocked && (await host.services.audio.requestPlayback()).granted;
const wasm = await WebAssembly.instantiateStreaming(fetch('/module.wasm'));
const sameOriginWasmWorked = wasm.instance instanceof WebAssembly.Instance;
const artifactHostWorked =
  (await fetch('/module.wasm?variant=1')).status === 404 &&
  (await fetch('/module.wasm', { method: 'POST' })).status === 405 &&
  (await fetch('/undeclared.bin')).status === 404;
host.services.telemetry.emit({ type: 'frameDurationMs', value: 16 });
let telemetryWorked = false;
try {
  host.services.telemetry.emit({
    type: 'frameDurationMs',
    value: 16,
    userId: 'forbidden',
  });
} catch {
  telemetryWorked = true;
}
await adapter.pause();
...
image.src = collector + '/image';
const script = document.createElement('script');
script.src = collector + '/script';
document.body.append(script);
const frame = document.createElement('iframe');
frame.src = collector + '/frame';
    ...
    parent.postMessage(
      {
        ...
        storageWorked,
        presentationWorked,
        presentationStateWorked,
        audioWorked,
        sameOriginWasmWorked,
        artifactHostWorked,
        telemetryWorked,
        lifecycleWorked: host.state === 'exit',
        listenersReleased: host.listenerCount === 0,
        blockedFetch,
        blockedDOM,
        absent,
        ownStorage: localStorage.getItem('akeru-conformance') === name,
        violations,
      },
      shell,
    ),
