import { createReferenceHost } from '/session.js';
...
setTimeout(() => parent.postMessage({ type: 'result', nonce, name, inputWorked, storageWorked, presentationWorked, telemetryWorked, lifecycleWorked: host.state === 'exit', listenersReleased: host.listenerCount === 0, blockedFetch, blockedDOM, absent, ownStorage: localStorage.getItem('akeru-conformance') === name, violations }, shell), 100);
