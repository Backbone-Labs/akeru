...
export function packageSource(cwd = process.cwd()) {
  ...
  if (git('status', '--porcelain', '--untracked-files=normal').length) {
    throw new Error(
      'Source packaging requires a clean checkout; commit or remove changes first.',
    );
  }
  const revision = git('rev-parse', 'HEAD').toString().trim();
  const files = git('ls-tree', '-rz', 'HEAD')
    .toString()
    .split('\0')
    .filter(Boolean)
    .map((entry) => {
      ...
      if (
        ...
        throw new Error(
          'Source archives permit only regular files; no symlinks or submodules.',
        );
      }
      const path = match[4];
      ...
      if (
        ...
        /\bexport-(?:ignore|subst)\b/u.test(bytes.toString('utf8'))
      ) {
        throw new Error(
          'Source archives cannot omit or rewrite committed files using export attributes.',
        );
      }
  ...
  if (process.versions.node !== expectedNode)
    throw new Error(`Use Node ${expectedNode} to package sources.`);
