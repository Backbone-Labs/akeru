...
export function validateAvailability(value) {
  if (!exactKeys(value, ['schemaVersion', 'generation', 'titles']) || value.schemaVersion !== '1.0.0'
    || !Number.isSafeInteger(value.generation) || value.generation < 0 || !Array.isArray(value.titles)) throw new Error('Invalid availability document');
  const ids = new Set();
  for (const title of value.titles) {
    if (!exactKeys(title, ['id', 'paused', 'current', 'releases']) || typeof title.id !== 'string' || !idPattern.test(title.id)
      || ids.has(title.id) || typeof title.paused !== 'boolean' || typeof title.current !== 'string' || !Array.isArray(title.releases)) throw new Error('Invalid title availability');
    ids.add(title.id);
    const digests = new Set();
    for (const release of title.releases) {
      if (!exactKeys(release, ['digest', 'version', 'saveSchemaVersion'])
        || typeof release.digest !== 'string' || !hashPattern.test(release.digest) || digests.has(release.digest)
        || typeof release.version !== 'string' || !versionPattern.test(release.version)
        || !Number.isSafeInteger(release.saveSchemaVersion) || release.saveSchemaVersion < 1) throw new Error('Invalid retained release');
      digests.add(release.digest);
    }
    if (!digests.has(title.current)) throw new Error('Current release must be retained');
  }
  return value;
}
...
export function changeAvailability(document, titleId, action, targetDigest) {
  ...
  if (action === 'pause') {
    ...
    if (target.saveSchemaVersion !== current.saveSchemaVersion) throw new Error('Save schema differs; reviewed migration required');
...
export function updateAvailabilityFile(path, titleId, action, digest) {
  ...
  try {
    const updated = changeAvailability(JSON.parse(readFileSync(absolute)), titleId, action, digest);
...
if (process.argv[1] && import.meta.url === pathToFileURL(resolve(process.argv[1])).href) {
  try {
    ...
    if (!path || !id || !action || extra.length) throw new Error('Usage: node scripts/availability.mjs <operator-file> <title-id> pause|rollback [digest]');
    console.log(JSON.stringify(updateAvailabilityFile(path, id, action, digest), null, 2));
