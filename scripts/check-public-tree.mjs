...
export function inspectPublicFile(path, content) {
  ...
  if (path.startsWith('/') || parts.some((part) => part === '..') || /[\\\x00-\x1f]/u.test(path)) {
    errors.push('unsafe repository path');
  }
  if (/^(research|tmp|node_modules|dist|coverage)(\/|$)/u.test(path)
    || /(^|\/)(\.env(?:\..*)?|.*\.(?:pem|key|p12|mobileprovision))$/u.test(path)
  ...
  if (/https:\/\/github\.com\/Backbone-Labs\/(?:ios|android|b3|cloud)(?:[\/#?]|$)/iu.test(content)
...
export function checkPublicTree(cwd = process.cwd()) {
  ...
  for (const path of paths) {
    ...
    for (const error of inspectPublicFile(path, readFileSync(fullPath, 'utf8'))) {
...
if (process.argv[1] && import.meta.url === pathToFileURL(resolve(process.argv[1])).href) {
  try {
    console.log(`Public tree checks passed (${checkPublicTree()} tracked files).`);
  } catch (error) {
