...
export function packageStaging(cwd = process.cwd()) {
  ...
  const html = committed('platform/staging/index.html')
    .toString()
  ...
  const files = {
    ...
    'source.tar.gz': readFileSync(
      resolve(cwd, 'dist/source', source.artifact.filename),
    ),
...
if (
  ...
  try {
    console.log(JSON.stringify(packageStaging(), null, 2));
  } catch (error) {
