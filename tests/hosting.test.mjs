...
test('hosted adapter restricts paths/methods, strips caller secrets and enforces headers', async () => {
  ...
  assert.equal(await page.text(), bytes);
  assert.equal(page.headers.get('set-cookie'), null);
  assert.equal(page.headers.get('access-control-allow-origin'), null);
  assert.equal(
    page.headers.get('content-security-policy'),
    shellHeaders['Content-Security-Policy'],
  );
  assert.equal(
    page.headers.get('strict-transport-security'),
    'max-age=31536000',
  );
  for (const path of ['/unknown', '/.env', '//index.html', '/%2findex.html'])
    assert.equal(
      (await worker.fetch(new Request(`https://example.invalid${path}`), env))
        .status,
      404,
    );
  assert.equal(
...
test('hosted adapter fails closed on changed bytes, redirects, missing assets and binding errors', async () => {
  for (const response of [
    ...
    assert.equal(
      (
        await worker.fetch(new Request('https://example.invalid/'), {
  ...
  assert.equal(
    (await worker.fetch(new Request('https://example.invalid/'), {})).status,
    503,
  );
});
...
test('health identifies exact release; HEAD never exposes body', async () => {
  ...
  const head = await worker.fetch(
    new Request('https://example.invalid/', { method: 'HEAD' }),
...
test('deployment verifier checks current source, headers and actual asset bytes', async () => {
  ...
  const result = await verifyHostedStaging(
  ...
  await assert.rejects(
    verifyHostedStaging('http://example.invalid', release, fetcher),
    /HTTPS/,
  );
  await assert.rejects(
    verifyHostedStaging(
      'https://example.invalid',
      { ...release, revision: 'c'.repeat(40) },
      fetcher,
    ),
    /revision/,
  );
