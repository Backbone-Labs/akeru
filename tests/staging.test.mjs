...
import { mkdtempSync, writeFileSync, readFileSync, rmSync, mkdirSync, cpSync, symlinkSync } from 'node:fs';
...
import { changeAvailability, updateAvailabilityFile } from '../scripts/availability.mjs';
...
function fixture(t) {
  ...
  cpSync(new URL('../platform/staging', import.meta.url), join(cwd, 'platform/staging'), { recursive: true });
...
test('staging output reproduces bytes, exposes revision and retains corresponding source', t => {
  ...
  assert.deepEqual(release, readFileSync(join(cwd, 'dist/staging/release.json')));
  const loaded = loadStaging(join(cwd, 'dist/staging'));
  assert.ok(loaded.files.get('/index.html').bytes.includes(first.revision));
  assert.equal(loaded.files.get('/LICENSE.txt').bytes.toString(), 'Original test fixture\n');
});
...
test('HTTP preview restricts methods and paths, returns revision health and shell security headers', async t => {
  ...
  assert.equal((await (await fetch(`${origin}/healthz`)).json()).revision, release.revision);
  for (const path of ['/research/notes.md', '/%2e%2e/LICENSE', '/.env', '//index.html', '/index.html/']) {
    assert.equal((await fetch(origin + path)).status, 404);
  }
  assert.equal((await fetch(origin, { method: 'POST' })).status, 405);
  assert.equal(await (await fetch(origin, { method: 'HEAD' })).text(), '');
  assert.equal((await fetch(`${origin}/source.tar.gz`)).headers.get('content-type'), 'application/gzip');
});

const current = 'a'.repeat(64), previous = 'b'.repeat(64);
const availability = () => ({ schemaVersion: '1.0.0', generation: 1, titles: [{ id: 'original-fixture', paused: false, current,
  releases: [{ digest: current, version: '1.1.0', saveSchemaVersion: 1 }, { digest: previous, version: '1.0.0', saveSchemaVersion: 1 }] }] });
...
test('operator can pause one title and roll back to retained compatible bytes without reactivation', () => {
  ...
  const rollback = changeAvailability(paused, 'original-fixture', 'rollback', previous);
  ...
  assert.deepEqual(changeAvailability(rollback, 'original-fixture', 'pause'), rollback);
});

test('rollback rejects unknown artifacts, unsafe save changes, duplicate titles and invented activation', () => {
  assert.throws(() => changeAvailability(availability(), 'original-fixture', 'rollback', 'c'.repeat(64)), /retained/);
  assert.throws(() => changeAvailability(availability(), 'original-fixture', 'activate'), /Only pause/);
  const mismatch = availability(); mismatch.titles[0].releases[1].saveSchemaVersion = 2;
  assert.throws(() => changeAvailability(mismatch, 'original-fixture', 'rollback', previous), /migration/);
  const duplicate = availability(); duplicate.titles.push(structuredClone(duplicate.titles[0]));
  assert.throws(() => changeAvailability(duplicate, 'original-fixture', 'pause'), /Invalid title/);
});
...
test('operator CLI preserves file on invalid update and prevents concurrent writers', t => {
  ...
  const bytes = JSON.stringify(availability()); writeFileSync(path, bytes);
  assert.throws(() => updateAvailabilityFile(path, 'missing', 'pause'), /Unknown/);
  ...
  assert.throws(() => updateAvailabilityFile(path, 'original-fixture', 'rollback', previous), /EEXIST/);
});
