  ...
  page: async ({ page }, use, testInfo) => {
    ...
    page.on('console', message => {
      if (message.type() === 'error') {
        ...
        failures.push(`console.error${location.url ? ` (${location.url}:${location.lineNumber + 1})` : ''}: ${message.text()}`);
      }
    });
...
export async function installSimulatedGamepad(page) {
  await page.addInitScript(() => {
    const state = { connected: true, buttons: Array(17).fill(0), axes: Array(4).fill(0) };
    const pad = {
    ...
    Object.defineProperties(pad, {
      buttons: { get: () => state.buttons.map(value => ({ value, pressed: value > 0.5, touched: value > 0 })) },
    ...
    Object.defineProperty(window, '__akeruTestGamepad', {
      configurable: false,
      value: {
        button(index, value) { state.buttons[index] = value; },
        axis(index, value) { state.axes[index] = value; },
        connect(value) { state.connected = value; },
        neutral() { state.buttons.fill(0); state.axes.fill(0); },
      },
    });
  });
}
...
export async function setGamepadButton(page, index, value) {
  await page.evaluate(([buttonIndex, buttonValue]) => window.__akeruTestGamepad.button(buttonIndex, buttonValue), [index, value]);
}
...
export async function expectNoHorizontalOverflow(page) {
  const result = await page.evaluate(() => ({
    ...
    offenders: [...document.querySelectorAll('body *')]
      ...
      .slice(0, 5)
      .map(element => `${element.tagName.toLowerCase()}${element.id ? `#${element.id}` : ''}${element.className ? `.${String(element.className).trim().replaceAll(/\s+/g, '.')}` : ''}`),
  }));
  expect(result.contentWidth, `Horizontal overflow from: ${result.offenders.join(', ')}`).toBeLessThanOrEqual(result.viewportWidth);
}
