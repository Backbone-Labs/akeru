...
test('browses from catalog to detail and launches the isolated original fixture', async ({ page }) => {
  ...
  await expect(page.getByRole('heading', { name: 'Orbit study' })).toBeVisible();

  await page.getByRole('button', { name: /Play now/ }).click();
  const runtime = page.frameLocator('iframe[title="Orbit study isolated runtime"]');
  await expect(runtime.getByRole('heading', { name: 'Find a little space.' })).toBeVisible();
  await expect(runtime.getByRole('status')).toHaveText('Ready when you are.');
  await expect(page.locator('#runtime-overlay')).toBeHidden();
});

test('holds and releases touch input while an idle controller remains connected', async ({ page }) => {
  ...
  await expect.poll(async () => (await orbPosition(runtime)).x).toBeGreaterThan(before.x + 5);
...
test('uses Start to pause and resume a running session', async ({ page }) => {
  ...
  await expect(page.getByRole('heading', { name: 'A little breather.' })).toBeVisible();
...
test('persists a per-title controller remap across reload', async ({ page }) => {
  await launchDemo(page, demo.url);
  ...
  await expect(runtime.getByRole('status')).toHaveText('Ready when you are.');
  await page.getByRole('button', { name: 'Controls' }).click();
  await expect(page.getByRole('combobox', { name: 'confirm control' })).toHaveValue('north');
});

test('requires a neutral controller after synthetic focus recovery', async ({ page }) => {
  await installSimulatedGamepad(page);
  const runtime = await launchDemo(page, demo.url);
  await page.waitForTimeout(100);

  await setGamepadButton(page, 15, 1);
  await expect(runtime.getByRole('status')).toHaveText('Controller input connected');
  await expect.poll(async () => (await orbPosition(runtime)).x).toBeGreaterThan(5);
  ...
  await page.waitForTimeout(100);
  await setGamepadButton(page, 15, 1);
  await expect.poll(async () => (await orbPosition(runtime)).x).toBeGreaterThan(blurred.x + 5);
  await neutralGamepad(page);
});

test('rejects a forged runtime message sent by the shell window', async ({ page }) => {
  await launchDemo(page, demo.url);
  ...
  await page.evaluate(value => {
    window.postMessage({ protocol: 'akeru.catalog.v1', nonce: value, sequence: Number.MAX_SAFE_INTEGER, type: 'exit', payload: {} }, location.origin);
  }, nonce);
  await page.waitForTimeout(100);
...
test('renders paused, unpublished, and unknown routes safely', async ({ page }) => {
  ...
  await page.evaluate(() => window.catalogPreview.navigate('/outside-the-catalog'));
  await expect(page.getByRole('heading', { name: 'A little off the map.' })).toBeVisible();
});

test('has no horizontal overflow through the mobile browse, detail, runtime, and controls flow', async ({ page }) => {
  ...
  await page.getByRole('button', { name: 'Controls' }).click();
  await expect(page.getByRole('dialog', { name: 'Control settings' })).toBeVisible();
  await expectNoHorizontalOverflow(page);
  const frameOverflow = await runtime.locator('html').evaluate(element => element.scrollWidth - element.clientWidth);
