...
test('standard gamepad values are bounded, mapped and deadzoned', () => {
  ...
  assert.deepEqual(
    [raw.axes.leftX, raw.axes.leftY, raw.axes.rightX, raw.axes.rightY],
    [0.1, -0.6, 1, -1],
  );
...
test('mapping accepts title actions but rejects unsupported sources and ambiguity', () => {
  assert.deepEqual(
    validateMapping('gamepad', {
      buttons: { south: 'fire' },
      axes: { leftX: 'steer' },
    }),
    { buttons: { south: 'fire' }, axes: { leftX: 'steer' } },
  );
  assert.throws(
    () =>
      validateMapping('gamepad', { buttons: { keyboardA: 'fire' }, axes: {} }),
    /Unsupported/,
  );
  assert.throws(
    () =>
      validateMapping('gamepad', {
        buttons: { south: 'fire', east: 'fire' },
        axes: {},
      }),
    /Ambiguous/,
  );
  assert.throws(
    () => validateMapping('touch', { buttons: {}, axes: { leftX: 'moveX' } }),
    /Unsupported/,
  );
});
...
test('preferences are validated, isolated by title and fail closed to defaults', () => {
  ...
  assert.throws(() =>
    validatePreferences({ ...defaultPreferences(), selectedController: 16 }),
  );
});
