...
test('public tree rejects internal paths and credentials, accepts ordinary source', () => {
  assert.deepEqual(
    inspectPublicFile('packages/example/index.js', 'export const value = 1;'),
    [],
  );
  assert.deepEqual(
    inspectPublicFile('.env.example', 'PUBLIC_ORIGIN=https://example.invalid'),
    [],
  );
  for (const path of [
    'research/notes.md',
  ...
  assert.ok(
    inspectPublicFile(
      'notes.md',
      'https://' + 'github.com/Backbone-Labs/' + 'ios/blob/main/file',
    ).length,
  );
});
...
test('same clean revision produces identical archive and complete file hashes', (t) => {
  ...
  const archive = readFileSync(
    join(cwd, 'dist/source', first.artifact.filename),
  );
  assert.equal(
  ...
  assert.equal(
    source.sha256,
    createHash('sha256').update('# Original fixture\n').digest('hex'),
  );
});
