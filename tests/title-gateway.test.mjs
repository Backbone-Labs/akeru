...
const previous = 'a'.repeat(64),
  current = 'b'.repeat(64);
const bytes = {
  'index.html': '<h1>Original fixture</h1>',
  'game.js': '/* Original fixture */',
};
const versions = [previous, current].map((digest) => ({
  digest,
  entry: 'index.html',
  files: Object.entries(bytes).map(([path, value]) => ({
    path,
...
const config = {
  titleId: 'fixture',
  titleOrigin: 'https://fixture.example.net',
  shellOrigin: 'https://shell.example.com',
  versions,
  headers: {
    ...shellHeaders,
    'Content-Security-Policy':
      "default-src 'none'; script-src 'self' 'wasm-unsafe-eval'; connect-src 'self'; frame-ancestors https://shell.example.com",
  },
};
function setup() {
  let availability = {
  ...
  const env = {
    REGISTRY: {
      fetch: async (request) => {
        ...
        return Response.json(availability.titles[0]);
      },
    },
    ASSETS: {
      fetch: async (request) =>
        new Response(bytes[new URL(request.url).pathname.split('/').at(-1)]),
    },
  };
  ...
  return {
    ...
    request: (path, options) =>
      worker.fetch(new Request(config.titleOrigin + path, options), env),
    update: (action, digest) => {
...
test('operator rollback changes new launches while active assets remain readable', async () => {
  ...
  assert.equal(
    (await f.request(`/releases/${current}/index.html`)).status,
    200,
  );
  ...
  assert.equal(
    retainedAsset.headers.get('cache-control'),
...
test('outage, wrong title state, unsupported methods and unknown paths fail closed', async () => {
  const f = setup();
  f.env.REGISTRY.fetch = async () => {
    throw new Error('offline');
  };
  assert.equal((await f.request('/')).status, 503);
  f.env.REGISTRY.fetch = async () =>
    Response.json({ id: 'other', paused: false, current });
  assert.equal((await f.request('/')).status, 503);
...
test('gateway rejects same-origin hosting and unsafe artifact configurations', () => {
  assert.throws(
    () => createTitleGateway({ ...config, titleOrigin: config.shellOrigin }),
    /isolated/,
  );
  assert.throws(
    () => createTitleGateway({ ...config, headers: shellHeaders }),
    /CSP/,
  );
  const bad = structuredClone(versions);
  bad[0].files[0].path = '../index.html';
  assert.throws(
    () => createTitleGateway({ ...config, versions: bad }),
    /artifact/,
  );
  for (const type of ['application/xhtml+xml', 'image/svg+xml', ' text/html']) {
    const active = structuredClone(versions);
    active[0].files[1].type = type;
    assert.throws(
      () => createTitleGateway({ ...config, versions: active }),
      /artifact/,
    );
  }
  const noEntry = structuredClone(versions);
  noEntry[0].entry = 'game.js';
  assert.throws(
    () => createTitleGateway({ ...config, versions: noEntry }),
    /HTML/,
  );
});
