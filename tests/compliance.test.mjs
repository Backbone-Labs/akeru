...
import {
  dossierDigest,
  policy,
  publicationEvidenceErrors,
  validateCandidateList,
  validateDossier,
} from '../scripts/validate-compliance.mjs';
...
const review = () => ({
  ...
  evidenceUrls: ['https://example.org/synthetic-review'],
});
function completed() {
  const dossier = draft();
  ...
  dossier.release = {
    ...
    sources: [
      {
        url: dossier.identity.upstreamUrl,
        revision: dossier.identity.observedRevision,
        role: 'game',
      },
    ],
  };
  for (const section of Object.values(dossier.sections)) {
    Object.assign(section, {
      status: 'complete',
      evidenceUrls: ['https://example.org/synthetic-evidence'],
      missingEvidence: [],
      review: review(),
    });
  }
  dossier.approval = {
    status: 'approved',
    review: review(),
    reviewedContentDigest: dossierDigest(dossier),
  };
  return dossier;
}
...
test('all 11 candidate dossiers validate but none has publication evidence', () => {
  ...
  assert.equal(
    candidates.find((dossier) => dossier.id === '2048').scope,
    'optional',
  );
  assert.equal(
    candidates.find((dossier) => dossier.id === 'hypersomnia').scope,
    'future',
  );
});
...
test('approval requires confirmed source, release, completed sections and accountable evidence', () => {
  const mutations = [
    ...
    (d) => {
      d.identity.status = 'candidate';
    },
    (d) => {
      d.release = null;
    },
    (d) => {
      d.sections.assets.status = 'unknown';
    },
    (d) => {
      d.sections.assets.missingEvidence = ['Music rights unresolved'];
    },
    (d) => {
      d.sections.assets.evidenceUrls = [];
    },
    (d) => {
      d.sections.assets.review = null;
    },
    (d) => {
      d.sections.assets.review.reviewer = ' ';
    },
    (d) => {
      d.approval.review.evidenceUrls = [];
    },
    (d) => {
      d.approval.review.reviewedAt = '2026-02-31T12:00:00Z';
    },
  ];
...
test('unknown, omitted or misspelled required sections fail closed', () => {
  for (const key of Object.keys(policy.requiredSections)) {
    const dossier = draft();
    delete dossier.sections[key];
    assert.notDeepEqual(validateDossier(dossier), []);
  }
  const dossier = draft();
  dossier.sections.licensing = dossier.sections.code;
...
test('floating pins, insecure sources and malformed release identities fail', () => {
  for (const revision of ['main', 'v1.0.0', 'abc1234', 'A'.repeat(40), null]) {
    const dossier = draft();
    dossier.identity.observedRevision = revision;
    assert.notDeepEqual(validateDossier(dossier), []);
  }
  for (const mutate of [
    (d) => {
      d.identity.upstreamUrl = 'http://example.org/source';
    },
    (d) => {
      d.identity.upstreamUrl = 'https://user:secret@example.org/source';
    },
    (d) => {
      d.release.sources[0].revision = 'latest';
    },
    (d) => {
      d.release.sources = [];
    },
    (d) => {
      d.release.sources[0].url = 'https://example.org/different-title';
    },
    (d) => {
      d.release.artifactDigest = 'sha256:abc';
    },
    (d) => {
      d.release.version = '01.0.0';
    },
  ]) {
    const dossier = completed();
    mutate(dossier);
...
test('changing release, source, evidence or policy invalidates recorded decision', () => {
  for (const mutate of [
    (d) => {
      d.release.artifactDigest = `sha256:${'b'.repeat(64)}`;
    },
    (d) => {
      d.identity.observedRevision = 'b'.repeat(40);
    },
    (d) => {
      d.sections.assets.finding = 'New asset included';
    },
    (d) => {
      d.sections.assets.review.reviewer = 'different-reviewer';
    },
    (d) => {
      d.policy.revision += 1;
    },
...
test('source and evidence URLs reject coercible non-string values', () => {
  for (const mutate of [
    (d) => {
      d.identity.upstreamUrl = [d.identity.upstreamUrl];
    },
    (d) => {
      d.release.sources[0].url = [d.release.sources[0].url];
    },
    (d) => {
      d.sections.assets.evidenceUrls = [['https://example.org/evidence']];
    },
    (d) => {
      d.sections.assets.review.evidenceUrls = [['https://example.org/review']];
    },
    (d) => {
      d.approval.review.evidenceUrls = [['https://example.org/review']];
    },
  ]) {
    const dossier = completed();
    mutate(dossier);
...
test('pending, rejected and unsupported policies do not become approval by default', () => {
  const dossier = draft();
  dossier.approval.status = 'rejected';
  dossier.approval.review = review();
  dossier.approval.reviewedContentDigest = dossierDigest(dossier);
  assert.deepEqual(validateDossier(dossier), []);
  assert.notDeepEqual(publicationEvidenceErrors(dossier), []);
...
test('malformed records and duplicate identities return errors', () => {
  for (const value of [null, [], {}, { approval: true }])
    assert.notDeepEqual(validateDossier(value), []);
  assert.notDeepEqual(validateCandidateList([draft(), draft()]), []);
  assert.notDeepEqual(validateCandidateList([]), []);
  const dossier = draft();
  dossier.release = { sources: [null] };
  assert.notDeepEqual(validateDossier(dossier), []);
});

test('digest is independent of object insertion order', () => {
  const dossier = draft();
  const reordered = Object.fromEntries(Object.entries(dossier).reverse());
