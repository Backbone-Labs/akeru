...
function gamepad(index, { buttons = {}, axes = {} } = {}) {
  ...
  for (const [name, value] of Object.entries(buttons))
    values[buttonIndexes[name]] = button(value);
  const axisValues = [0, 0, 0, 0];
  for (const [name, value] of Object.entries(axes))
    axisValues[{ leftX: 0, leftY: 1, rightX: 2, rightY: 3 }[name]] = value;
  return {
    index,
    connected: true,
    mapping: 'standard',
...
function environment({ pads = [], storage, now = { value: 0 } } = {}) {
  ...
  return {
    ...
    roots: {
      touchRoot: new FakeElement('div'),
      controlsRoot: new FakeElement('div'),
    },
  };
}
...
test('mandatory touch supports capture, cancel and focus release while idle pads stay inactive', () => {
  ...
  const seen = [],
    navigation = [];
  provider.subscribe((value) => seen.push(value));
  provider.subscribeNavigation((value) => navigation.push(value));
  const confirm = descendants(env.roots.touchRoot).find(
    (node) => node.dataset.control === 'south',
  );
  confirm.dispatch('pointerdown', { pointerId: 3 });
  assert.equal(seen.length, 0, 'mounted controls remain inactive until start');
  provider.start();
  const down = confirm.dispatch('pointerdown', { pointerId: 4 });
  assert.equal(down.defaultPrevented, true);
  assert.equal(confirm.capturedPointer, 4);
  assert.equal(seen.at(-1).buttons.confirm, 1);
  assert.ok(Object.isFrozen(seen.at(-1).buttons));
  ...
  assert.equal(
    seen.length,
    count,
    'an idle controller must not release held touch',
  );
  assert.equal(
    navigation.length,
    0,
    'mounted gameplay never emits controller navigation',
  );
  confirm.dispatch('pointercancel', { pointerId: 4 });
  assert.equal(seen.at(-1).buttons.confirm, 0);
  confirm.dispatch('pointerdown', { pointerId: 5 });
  env.window.dispatch('blur');
  assert.deepEqual(seen.at(-1).buttons, {});
  env.window.dispatch('focus');
  env.frames.step();
  assert.notEqual(seen.at(-1).buttons.confirm, 1);
  provider.stop();
  const stopped = seen.length;
  confirm.dispatch('pointerdown', { pointerId: 6 });
  assert.equal(seen.length, stopped);
  provider.dispose();
  provider.dispose();
  assert.equal(env.roots.touchRoot.children.length, 0);
});

test('multiple controllers select active input and reconnect only after neutral', () => {
  const pads = [gamepad(0), gamepad(1)],
    env = environment({ pads });
  const provider = createBrowserInputProvider({
    titleId: 'pad-title',
    window: env.window,
    document: env.document,
  ...
  provider.start();
  env.frames.step();
  pads[1] = gamepad(1, { buttons: { south: 1 } });
  env.frames.step();
  assert.equal(seen.length, 0, 'selection-time held input is gated');
  pads[1] = gamepad(1, { axes: { leftX: 0.1 } });
  env.frames.step();
  pads[1] = gamepad(1, { buttons: { south: 1 } });
  env.frames.step();
  assert.equal(provider.getState().activeController, 1);
  assert.equal(seen.at(-1).buttons.confirm, 1);
  pads.length = 1;
  env.frames.step();
  assert.equal(seen.at(-1).connected, false);
  pads[1] = gamepad(1, { buttons: { south: 1 } });
  provider.selectController(1);
  env.frames.step();
...
test('catalog gamepad navigation uses edges and bounded directional repeat', () => {
  ...
  env.frames.step();
  clock.value = 10;
  pads[0] = gamepad(0, { buttons: { dpadRight: 1, south: 1 } });
  env.frames.step();
  ...
  clock.value = 370;
  env.frames.step();
  assert.deepEqual(navigation.at(-1), { type: 'move', direction: 'right' });
  clock.value = 500;
  env.frames.step();
  assert.equal(
    navigation.filter((event) => event.type === 'activate').length,
    1,
  );
  provider.dispose();
});
...
test('keyboard navigation is shell-only and ignores edits and modifier chords', () => {
  ...
  const navigation = [],
    snapshots = [];
  provider.subscribeNavigation((value) => navigation.push(value));
  provider.subscribe((value) => snapshots.push(value));
  provider.start();
  const handled = env.window.dispatch('keydown', {
    key: 'ArrowDown',
    target: new FakeElement('div'),
  });
  env.window.dispatch('keydown', {
    key: 'Enter',
  ...
  assert.deepEqual(navigation, [{ type: 'move', direction: 'down' }]);
...
test('restricted storage and gamepad APIs degrade to usable touch and keyboard', () => {
  ...
  confirm.dispatch('pointerdown', { pointerId: 1 });
  assert.equal(seen.at(-1).buttons.confirm, 1);
...
test('preferences persist only in their validated title scope', () => {
  ...
  const create = (titleId) =>
    createBrowserInputProvider({
      titleId,
      window: env.window,
      document: env.document,
      navigator: env.navigator,
  ...
  one.setDeadzone(0.3);
  one.setMapping('gamepad', {
    buttons: { south: 'fire' },
    axes: { leftX: 'steer' },
  });
  one.selectController(2);
  one.dispose();
  const reloaded = create('one'),
    other = create('two');
  assert.equal(reloaded.getPreferences().deadzone, 0.3);
  assert.equal(reloaded.getPreferences().selectedController, 2);
  assert.equal(
    reloaded.getPreferences().mappings.gamepad.buttons.south,
    'fire',
  );
...
test('generic controls overlay remaps baseline actions without dropping title actions', () => {
  ...
  const provider = createBrowserInputProvider({
    ...
    cancelAnimationFrame: env.frames.cancel,
    mapping: {
      gamepad: { buttons: { south: 'fire' }, axes: { leftX: 'steer' } },
    },
  });
  provider.mount(env.roots);
  const confirm = descendants(env.roots.controlsRoot).find(
    (node) => node.dataset.action === 'confirm',
  );
  confirm.value = 'east';
  confirm.dispatch('change');
  const mapping = provider.getPreferences().mappings.gamepad;
  assert.equal(mapping.buttons.south, 'fire');
  assert.equal(mapping.buttons.east, 'confirm');
  assert.equal(mapping.axes.leftX, 'steer');
  provider.dispose();
});
...
test('stopped controls can refresh hotplug state without input or navigation', () => {
  ...
  pads.push(gamepad(2));
  assert.deepEqual(
    provider.refreshControllers().map((controller) => controller.index),
    [0, 2],
  );
  assert.deepEqual(
    provider.getState().controllers.map((controller) => controller.index),
    [0, 2],
  );
