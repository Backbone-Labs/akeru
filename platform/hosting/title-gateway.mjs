...
export function createTitleGateway({
  ...
  if (
    !/^[a-z0-9]+(?:-[a-z0-9]+)*$/u.test(titleId) ||
    !canonical(titleOrigin) ||
    !canonical(shellOrigin) ||
    titleOrigin === shellOrigin
  )
    throw new Error('Invalid isolated title identity');
  const ancestors = headers?.['Content-Security-Policy']
    ?.split(';')
    .map((s) => s.trim())
    .find((s) => s.startsWith('frame-ancestors '));
  if (ancestors !== `frame-ancestors ${shellOrigin}`)
    throw new Error('Title CSP must bind the expected shell');
  if (!Array.isArray(versions) || !versions.length)
    throw new Error('No retained releases');
  ...
  for (const version of versions) {
    if (
      !/^[a-f0-9]{64}$/u.test(version.digest) ||
      retained.has(version.digest) ||
    ...
    for (const file of version.files) {
      if (
        ...
        throw new Error('Invalid retained artifact');
      paths.add(file.path);
      ...
      if (file.type.toLowerCase().startsWith('text/html'))
        entries.set(`/${path}`, version.digest);
    }
    const entry = `/releases/${version.digest}/${version.entry}`;
    if (entries.get(entry) !== version.digest)
      throw new Error('Entry must be a retained HTML artifact');
      ...
      if (url.pathname === '/' || entries.has(url.pathname)) {
        ...
        try {
          const response = await env.REGISTRY.fetch(
            new Request(`https://registry.invalid/titles/${titleId}`, {
              redirect: 'manual',
          ...
          if (
            ...
            !retained.has(current.current)
          )
            throw new Error('Invalid title state');
        } catch {
          return respond(503, 'Title state unavailable');
        }
        if (current.paused) return respond(410, 'Title unavailable');
        if (url.pathname === '/')
          return respond(302, '', { Location: retained.get(current.current) });
        if (entries.get(url.pathname) !== current.current)
          return respond(409, 'Release changed; launch again');
      } else if (!assetPaths.has(url.pathname))
        return respond(404, 'Not found');
      const response = await assets.fetch(request, env);
      if (response.status === 200 && !entries.has(url.pathname))
