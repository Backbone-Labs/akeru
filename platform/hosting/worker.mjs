  ...
  return {
    async fetch(request, env) {
      ...
      const respond = (status, body, type = 'text/plain; charset=utf-8') => new Response(request.method === 'HEAD' ? null : body, {
        status, headers: { ...headers, 'Content-Type': type, 'Cache-Control': 'no-store',
          ...(url.protocol === 'https:' ? { 'Strict-Transport-Security': 'max-age=31536000' } : {}) },
      });
      if (!['GET', 'HEAD'].includes(request.method)) return respond(405, 'Method not allowed');
      if (url.pathname === '/healthz') return respond(200, JSON.stringify({
        status: 'ok', revision: release.revision, releaseDigest: release.digest, gamesEnabled: false,
      }), 'application/json');
      ...
      try {
        ...
        if (bytes.byteLength !== file.size || hash !== file.sha256) return respond(503, 'Release unavailable');
        return respond(200, bytes, file.type);
      } catch { return respond(503, 'Release unavailable'); }
    },
  };
}
