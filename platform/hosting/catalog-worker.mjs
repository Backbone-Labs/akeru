...
export function catalogHeaders(titleOrigins = []) {
  if (!Array.isArray(titleOrigins) || titleOrigins.some(origin => {
  ...
  return {
    ...
    'Permissions-Policy': 'camera=(), microphone=(), geolocation=(), payment=(), usb=(), gamepad=(self)',
...
export function createCatalogWorker(release) {
  ...
  if (!/^[a-f0-9]{40}$/.test(release.revision) || !/^[a-f0-9]{64}$/.test(release.digest)) throw new Error('Invalid release identity');
  const types = new Set(['text/html; charset=utf-8', 'text/css; charset=utf-8', 'text/javascript; charset=utf-8', 'application/json; charset=utf-8', 'text/plain; charset=utf-8', 'application/gzip']);
  const files = new Map();
  for (const file of release.files) {
    if (!/^[a-zA-Z0-9_-]+(?:\/[a-zA-Z0-9_-]+)*\.[a-zA-Z0-9.]+$/.test(file.path) || file.path.includes('..') || files.has(`/${file.path}`)
      || !types.has(file.type) || !Number.isSafeInteger(file.size) || file.size < 0 || !/^[a-f0-9]{64}$/.test(file.sha256)) throw new Error('Invalid release file');
    files.set(`/${file.path}`, file);
  }
  if (!files.has('/index.html') || !files.has('/catalog.json')) throw new Error('Missing catalog entry');
  return { async fetch(request, env) {
    const url = new URL(request.url);
    const respond = (status, body, type = 'text/plain; charset=utf-8') => new Response(request.method === 'HEAD' ? null : body, {
      status, headers: { ...headers, 'Content-Type': type, ...(url.protocol === 'https:' ? { 'Strict-Transport-Security': 'max-age=31536000' } : {}) },
    });
    ...
    try {
      ...
      if (bytes.byteLength !== file.size || digest !== file.sha256) return respond(503, 'Catalog unavailable');
      return respond(200, bytes, file.type);
    } catch { return respond(503, 'Catalog unavailable'); }
  } };
}
